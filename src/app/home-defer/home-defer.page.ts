...
@Component({
  ...
  standalone: true,
  imports: [IonSearchbar, IonBadge, IonLabel, IonAvatar, IonItem, IonList, IonHeader, IonToolbar, IonTitle, IonContent, IonList,IonSkeletonText,IonAlert,NgIf,NgFor,DatePipe,RouterModule,RouterLink,IonInfiniteScroll,IonInfiniteScrollContent],
})
export class HomeDeferPage {
  ...
  onSearchChange(searchTerm: string | null | undefined) {
    if (searchTerm && searchTerm.trim() !== '') {
      this.movieService.searchMovies(searchTerm).subscribe((results) => {
        this.moviess = results;
      });
    } else {
      this.loadMovies();
    }
